...
Switching between `buffers_mode=2` and `buffers_mode=1` in PyCO2SYS does not alter this figure sufficiently for any differences to be visible.
...
With `buffers_mode=2`, PyCO2SYS does use the finite-difference approach but it reduces the DIC perturbation size from 1 to 0.01 μmol/kg-sw to increase its accuracy.  If we revert both this change and the corrections above, then PyCO2SYS returns Revelle factors that agree with CO2SYS for MATLAB to within 10<sup>−6</sup>%.
