...
def KB(TempK, Sal, Pbar, WhichKs, fH, SWStoTOT0):
    ...
    KB = where((WhichKs==6) | (WhichKs==7),
        p1atm.kBOH3_NBS_LTB69(TempK, Sal)/fH, KB) # convert NBS to SWS
    KB = where((WhichKs!=6) & (WhichKs!=7) & (WhichKs!=8),
        p1atm.kBOH3_TOT_D90b(TempK, Sal)/SWStoTOT0, KB) # convert TOT to SWS
    # Now correct for seawater pressure
...
def KW(TempK, Sal, Pbar, WhichKs):
    ...
    KW = where((WhichKs!=6) & (WhichKs!=7) & (WhichKs!=8),
        p1atm.kH2O_SWS_M95(TempK, Sal), KW)
    # Now correct for seawater pressure
...
def KSi(TempK, Sal, Pbar, WhichKs, fH):
    ...
    KSi = where(WhichKs==7,
        p1atm.kSi_NBS_SMB64(TempK, Sal)/fH, KSi) # convert NBS to SWS
    # Note: neither the GEOSECS choice nor the freshwater choice include
    # contributions from phosphate or silicate.
    KSi = where((WhichKs==6) | (WhichKs==8), 0.0, KSi)
    KSi = where((WhichKs!=6) & (WhichKs!=7) & (WhichKs!=8),
        p1atm.kSi_SWS_YM95(TempK, Sal), KSi)
    # Now correct for seawater pressure
...
def KH2S(TempK, Sal, Pbar, WhichKs, SWStoTOT0):
    ...
    KH2S = where((WhichKs==6) | (WhichKs==7) | (WhichKs==8), 0.0,
        p1atm.kH2S_TOT_YM95(TempK, Sal)/SWStoTOT0) # convert TOT to SWS
    # Now correct for seawater pressure
...
def KNH3(TempK, Sal, Pbar, WhichKs, SWStoTOT0):
    ...
    KNH3 = where((WhichKs==6) | (WhichKs==7) | (WhichKs==8), 0.0,
        p1atm.kNH3_TOT_CW95(TempK, Sal)/SWStoTOT0) # convert TOT to SWS
    # Now correct for seawater pressure
...
def KC(TempK, Sal, Pbar, WhichKs, fH, SWStoTOT0):
    ...
    K1, K2 = _getKC((WhichKs==6) | (WhichKs==7), p1atm.kH2CO3_NBS_MCHP73, fH,
                       K1, K2, ts)
